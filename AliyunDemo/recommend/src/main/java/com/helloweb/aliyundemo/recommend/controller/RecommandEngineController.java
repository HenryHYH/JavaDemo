...
import com.helloweb.aliyundemo.recommend.sdk.LogRequest;
import com.helloweb.aliyundemo.recommend.sdk.LogResponse;
...
public class RecommandEngineController {
	...
	@ApiOperation(value = "日志API", produces = "application/json", httpMethod = "POST")
	@ApiImplicitParam(name = "request", value = "请求实体", required = true, dataType = "LogRequest")
	@RequestMapping(path = "/logging", produces = "application/json", method = RequestMethod.POST)
	public LogResponse logging(@RequestBody LogRequest request) {

		LogResponse response = validate(request, LogResponse.class);
		if (null == response)
			return factory.logging(request);
		else
			return response;
	}
	...
	public RecommandResponse recommand(@RequestBody RecommandRequest request) {
